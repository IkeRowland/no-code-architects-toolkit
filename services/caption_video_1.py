...
def process_subtitle_content(content, one_word_highlight, single_word_display):
        ...
        if line.strip() and '-->' in line:
            ...
            if single_word_display:
                for i, word in enumerate(words):
                    ...
                    processed_lines.append(f"{{\\highlight}}{word}")
                ...
                for i, word in enumerate(words):
                    word_start = start_time + timedelta(seconds=i*word_duration)
                    word_end = word_start + timedelta(seconds=word_duration)
                    processed_lines.append(f"Dialogue: 0,{word_start.strftime('%H:%M:%S,%f')[:-3]},{word_end.strftime('%H:%M:%S,%f')[:-3]},Default,,0,0,0,,{{\\highlight}}{word}")
        else:
            processed_lines.append(line)
...
def process_captioning(file_url, caption_srt, caption_type, options, job_id):
    ...
    try:
        ...
        single_word_display = options.get('single_word_display', False)
        ...
        if caption_srt.startswith("https"):
            ...
            if caption_type in ['srt','vtt']:
                ...
                subtitle_content = process_subtitle_content(subtitle_content, one_word_highlight, single_word_display)
